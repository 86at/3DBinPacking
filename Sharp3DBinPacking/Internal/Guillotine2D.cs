...
public class Guillotine2D
{
        ...
        out int freeRectIndex)
    {
        ...
        for (var index = 0; index < _freeRectangles.Count; ++index)
        {
            ...
            if (width <= freeRectangle.Width &&
                height <= freeRectangle.Height)
            {
                var score = ScoreByHeuristic(rect, freeRectangle, rectChoice);
        ...
        Rectangle freeRect)
    {
        ...
        var leftoverVert = Math.Abs(freeRect.Width - rect.Width);
